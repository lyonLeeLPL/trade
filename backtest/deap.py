def eval_backtest(individual):
    class VixCci:
        ...
        df = pd.read_csv('data/ohlcv/20180820.csv', header=None)
        high = np.array(df[2])
        low = np.array(df[3])
        close = np.array(df[4])
        ...
        def cci_signal_long(self, tcci, ccis):
            ...
            for i in range(1, len(tcci)-1):
                if (tcci[i-1] < ccis[i-1] and tcci[i] > ccis[i]) or (ccis[i-1] < 0 and ccis[i] > 0):
                    signal.append(1)
                else:
                    signal.append(0)
            return signal
    ...
    vix_signal_long = vix.vix_signal_long(wvf, upperBand, rangeHigh)
    cci_signal_long = vix.cci_signal_long(tcci, ccis)

    vix_signal_short = vix.vix_signal_short(wvf_inv, lowerBand, rangeLow)
    ...
    def judge_vix(signal_long, signal_long_close, signal_short, signal_short_close, data=data):
        judge = [0] * len(signal_long)
        num = 0
        price = []
        for i in range(len(signal_long)-1):
            if num <= 0 and num > -5 and signal_long[i] == 1:
                num += -1
                judge[i] += -1
                price.append(data[i]-data[i+2])
            elif num >= 0 and num < 5 and signal_short[i] == 1:
                num += 1
                judge[i] += 1
                price.append(data[i+2]-data[i])
            elif num > 0:
    ...
    position = 0
    number_of_trade = 0
    judge, price = judge_vix(signal_long, signal_long_close, signal_short, signal_short_close)
    total1 = [10000]
    total2 = []
