...
def buy_li(price)
  begin
    return p $client.my_send_child_order(product_code: "FX_BTC_JPY", child_order_type: "LIMIT", price: price, side: "BUY", size: 0.03, minute_to_expire: 43200, time_in_force: "GTC")
  rescue => e
...
def sell_li(price)
  begin
    return p $client.my_send_child_order(product_code: "FX_BTC_JPY", child_order_type: "LIMIT", price: price, side: "SELL", size: 0.03, minute_to_expire: 43200, time_in_force: "GTC")
  rescue => e
    puts "エラーが起き、買えませんでした。2秒待機します"
    sleep(2)
  end
...
#main-------------------------------------------------------------------------------------------
sa_pre, ikioi_pre, i, pos = 0, 0, 0, 0
while true
  sa = inago_sa
  puts "差:#{sa}, 前の差:#{sa_pre} pos:#{pos}"
  if pos == 1
    if sa < 30 || sa > 500
