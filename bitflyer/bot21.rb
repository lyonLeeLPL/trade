...
def orderbuy(buyprice, sellprice, volume)
  begin
...
def ordersell(sellprice, buyprice, volume)
  begin
    p $client.my_send_child_order(product_code: "FX_BTC_JPY", child_order_type: "LIMIT", price: sellprice, side: "SELL", size: volume, minute_to_expire: 43200, time_in_force: "GTC")
    p $client.my_send_child_order(product_code: "FX_BTC_JPY", child_order_type: "LIMIT", price: buyprice, side: "BUY", size: volume, minute_to_expire: 43200, time_in_force: "GTC")
  rescue => e
...
while true
  ...
  p buyprice = mid_price - (sa/2).round
  p sellprice = mid_price + (sa/2).round
  # ordersell(sellprice, buyprice, 0.05)
  # $client.executions(product_code: "FX_BTC_JPY", count: 100, before: nil, after: nil).map do |item|
  #   sell += item["size"] if item["side"]=='SELL'
  #   buy += item["size"] if item["side"]=='BUY'
  # end
  ...
  #   cancel
  #   sell_li(mid_price, position_size)
  #   puts "買い決済"
  # elsif position_side == "SELL"
  #   cancel
  #   buy_li(mid_price, position_size)
  #   puts "売り決済"
  # end
  if $client.health["status"] == "NORMAL" && position_size < 0.01
    if bid_size_5 < ask_size_5
      cancel
      if pos == 1
        ordersell(mid_price+10, mid_price-50, position_size)
      elsif pos == -1
        ordersell(mid_price+10, mid_price-50, position_size/2)
      else
        ordersell(mid_price+10, mid_price-50, 0.04)
      end
      puts "売り"
      i = 1
    else
      cancel
      if pos == -1
        orderbuy(mid_price-10, mid_price+50, position_size)
      elsif pos == 1
        orderbuy(mid_price-10, mid_price+50, position_size/2)
      else
        orderbuy(mid_price-10, mid_price+50, 0.04)
      end
      puts "買い"
      i = 0
    end
