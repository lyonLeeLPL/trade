...
def order(buyprice, sellprice, volume)
  begin
...
while true
  ...
  size_ask = 0
  pri_ask = 0
  $client.board(product_code: "FX_BTC_JPY")["asks"].take(15).map do |item|
  ...
  size_bid = 0
  pri_bid = 0
  $client.board(product_code: "FX_BTC_JPY")["bids"].take(15).map do |item|
  ...
  puts pri_bid
  cancel
  order(pri_bid+1, pri_ask-1, 0.01)

  if position_size >= 0.01
    # if position_side == "BUY"
    #   if bid_size_5 > ask_size_5
    order(pri_bid+1, pri_ask-1, position_size)
    #     puts "買い指値1"
    #   else
