...
price_ima = 0
price_before = 0
CSV.foreach('ita1.csv') do |ita|
  price_ima = (ita[3].to_i+ita[2].to_i)/2
  if (bidsize_before < asksize_before) && (price_ima < price_before)
    count += 1
  end
  price_before = (ita[3].to_i+ita[2].to_i)/2
  bidsize_before = ita[8].to_i
  asksize_before = ita[9].to_i
end
p count
