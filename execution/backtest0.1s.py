    ...
    for data in reader:
        ...
        for o in order_list[:]:
            if o[2] > 5:
                ...
                order += -1
            o[2] += 1
        sa = abs(buy_size-sell_size)
        if sa > 5:
            order_list.clear()
        elif buy_size - sell_size > 0 and order <= 3 :
            order_list.append([best_bid-(sa*100), "BUY", 0])
        elif sell_size - buy_size > 0 and order >= -3:
            order_list.append([best_ask+(sa*100), "SELL", 0])
    amount = 0
    order = 0
    max_order = 0
    min_order = 0
