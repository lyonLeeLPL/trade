...
while true
  ...
  if position_size <= 0.01
    if bid_size_5 > ask_size_5
      orderbuy(mid_price, pri_ask-1, pri_bid-1, 0.01, 0.01)
      puts "買い指値"
    else
      ordersell(mid_price, pri_bid+1, pri_ask+1, 0.01, 0.01)
      puts "売り指値"
    end
    i += 1
  end
  if position_size >= 0.01
    if position_side == "BUY"
      if size_bid > size_ask
        orderbuy(mid_price, pri_ask-1, pri_bid-1, 0.01, position_size+0.01)
        puts "買い指値"
      else
        ordersell(mid_price, pri_bid+1, pri_ask+1, position_size+0.01, 0.01)
        puts "売り指値"
      end
    end
    if position_side == "SELL"
      if size_bid > size_ask
        orderbuy(mid_price, pri_ask-1, pri_bid-1, position_size+0.01, 0.01)
        puts "買い指値"
      else
        ordersell(mid_price, pri_bid+1, pri_ask+1, 0.01, position_size+0.01)
        puts "売り指値"
      end
    end
    i += 1
